...
import type {EventId, EventSearchResult, MyContext} from '../../lib/types.ts';
...
function findEvents(ctx: MyContext): Readonly<EventSearchResult> {
	const filter = ctx.session.eventfilter;
...
menu.choose('a', {
	...
	async choices(ctx) {
		try {
			...
			return Object.fromEntries([
				...Object.entries(filteredEvents.subDirectories)
					.map(([name, [directory, path]]) =>
						directory.subDirectories !== undefined || directory.events !== undefined
							? ['d:' + path.join(':'), '🗂️ ' + name]
							: ['dx:' + path.join(':'), '🚫 ' + name]),
				...Object.entries(filteredEvents.events)
					.map(([eventId, name]) =>
	...
	async do(ctx, key) {
		if (key.startsWith('e:')) {
			const eventId = key.slice(2) as EventId;
		...
		if (key.startsWith('d:')) {
			ctx.session.eventPath = key.slice(2).split(':');
